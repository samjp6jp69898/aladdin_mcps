 ...
 * 錯誤，企劃端的 Claude Code 只會顯示「連線失敗」）；其他例外的處理必須一字不變。
 *
 ...
 */
const testDir = mkdtempSync(join(tmpdir(), 'agrabah-platform-http-test-'));
const auditLogPath = join(testDir, 'audit.jsonl');
const TOOL_NAME = 'agrabah_platform_list_game_vendors';

const childScript = `
const DIR = ${ JSON.stringify(dirname(fileURLToPath(import.meta.url))) };
const { withStderrStackLogging } = await import(DIR + '/http.ts');
const { ReloginRequiredError } = await import(DIR + '/session.ts');
...
const auditLines = (await Bun.file(process.env.AGRABAH_PLATFORM_AUDIT_LOG_PATH).text())
    .trim().split('\\n').map(line => JSON.parse(line));

await Bun.write(Bun.stdout, JSON.stringify({ relogin, unexpected, ok, auditLines }));
// 顯式結束：files.ts 在模組載入時就掛了一個週期性清理的 setInterval（files.ts:348），
// 子行程不會自己結束，spawnSync 會一直等下去。
process.exit(0);
`;
...
const child = spawnSync('bun', [ '-e', childScript ], {
    ...
    env: {
        ...
        // session.ts 對 AGRABAH_PLATFORM_API_URL 的啟動檢查。
        AGRABAH_PLATFORM_API_URL: 'http://127.0.0.1:1/never-called-in-this-test',
        AGRABAH_PLATFORM_AUDIT_LOG_PATH: auditLogPath,
        ...
        // 斷言會隨測試檔執行順序時好時壞。
        AGRABAH_PLATFORM_AUDIT_LOG_MAX_BYTES: String(10 * 1024 * 1024),
        AGRABAH_PLATFORM_FILES_TMP_DIR: join(testDir, 'tmp-uploads'),
    },
    timeout: 60_000,
});
...
describe('withStderrStackLogging — 其他結果（行為不得改變）', () => {
    ...
    test('成功回傳的 tool 不受影響，稽核仍記 success', () => {
        expect(observed.auditLines[ 2 ]!.result).toBe('success');
    });
});
