/**
 * tools/login.ts — agrabah_platform_login
 */
...
export function registerLoginTool(server: McpServer): void {
    server.registerTool(
        'agrabah_platform_login',
        {
            title: 'Login to agrabah platform backend',
            description:
                '登入 agrabah platform 後台（rajah: Auth.Login），取得後續 RPC 呼叫所需的登入態。' +
            ...
            inputSchema: {
                identifier: z.string().optional().describe('登入帳號，留空則用環境變數 AGRABAH_PLATFORM_USER'),
                password: z.string().optional().describe('登入密碼，留空則用環境變數 AGRABAH_PLATFORM_PASSWORD'),
