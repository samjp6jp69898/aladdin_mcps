/**
 * tools/list_game_vendors.ts — agrabah_platform_list_game_vendors
 *
...
export function registerListGameVendorsTool(server: McpServer): void {
    server.registerTool(
        'agrabah_platform_list_game_vendors',
        {
            ...
            description:
                ...
                '否則走 ListGameVendors 分頁查詢。回傳的 id 就是 agrabah_platform_list_vendor_games 要用的 gameVendorId。' +
                '注意：透過 agrabah-admin 剛建立的場館不會自動出現在這裡——場館要先被 admin 端啟用給特定 platform' +
