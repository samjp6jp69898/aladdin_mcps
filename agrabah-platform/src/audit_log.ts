/**
 * audit_log.ts — H32 在 agrabah-admin/src/audit_log.ts 拍板的稽核 log 契約，
 ...
 * （AGRABAH_PLATFORM_AUDIT_LOG_MAX_BYTES，預設 10MB）就 closeSync 現有 fd、
...
const LOG_PATH = process.env.AGRABAH_PLATFORM_AUDIT_LOG_PATH
    ?? new URL('../logs/audit.jsonl', import.meta.url).pathname;
const MAX_BYTES = Number(process.env.AGRABAH_PLATFORM_AUDIT_LOG_MAX_BYTES ?? 10 * 1024 * 1024); // 10MB
