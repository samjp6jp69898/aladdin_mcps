 ...
 * 不重寫任何 tool 程式碼。
 *
 * 契約（H1 在 agrabah-admin/src/http.ts 拍板，本檔沿用）：
 ...
 * agrabah-admin/src/auth.ts 定型契約的原樣複製）。
 *
...
// agrabah-admin/tokens.json 是分開的兩份檔案，互不相交（D11 權限隔離）。
const TOKENS_PATH = process.env.AGRABAH_PLATFORM_TOKENS_PATH
 ...
 * obsidian/mcps/agrabah-admin/src/http.ts 同一段註解與 ./files.ts 檔頭
...
app.onError((err, c) => {
    console.error(`[agrabah-platform http] unhandled error: ${ err instanceof Error ? (err.stack ?? err.message) : String(err) }`);
    return c.text('Internal Server Error', 500);
});

console.error(`[agrabah-platform MCP] http server ready on 127.0.0.1:${ PORT }`);
