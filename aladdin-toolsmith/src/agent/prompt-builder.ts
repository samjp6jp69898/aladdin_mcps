...
- 如果這支 method 屬於 method-category-checklist.md 裡的高風險分類（例如讀取清單
  類、**搜尋欄位哨兵值類（第 2.5 節）**、Upsert 類、業務鍵間接定位更新類），知道
  該分類要求的具體檢查項在這次需求裡該怎麼落實？
...
1. **查清楚目標 RPC method（不能用猜的）**：讀 rajah/services/*.rajah 或用該文件
   ...
   **只要這支 method 吃 search / 篩選條件，第 2.5 節（哨兵值陷阱）一律要過**：
   對 search 模型裡你沒有帶的每一個欄位，都要去 agrabah 讀出後端實際怎麼判斷它，
   確認「不篩選」的語意真的等於 protobuf 預設值（數字 0 / 空字串 / false）。
   2026-09-02 的真實 bug 就是漏了這一步——list_games 沒帶 displayTag/rebateTag，
   後端契約是 -1 才代表全部、0 是合法分類值，於是這支 tool 對任何廠商都回空陣列，
   而 RPC 一路回 success，完全看不出是查詢寫錯了。
...
5. **真的在 verify-workspace 這份副本上跑一次 dev 驗證**（例如
   ...
   有 search 的 tool 要驗「完全不帶任何選填篩選條件」的呼叫，且筆數要跟後台畫面
   對得上——只驗「帶滿條件」的情境會剛好蓋掉哨兵值缺陷），
