...
// 原因：這段直接讀寫共用的正式目錄（REAL_DIR[target]）跟同一個 aladdin_mcps
// git repo，admin/platform 兩個 target 也共用同一個 repo 的 git index/HEAD，不能
...
// aladdin_mcps 是 toolsmith 自己所在的 git repo，也是 aladdin-admin/aladdin-platform
// 正式目錄的所在 repo（2026-08-31 從 obsidian 拆出來，路徑攤平不再有 mcps/
// 前綴）——deploy-pipeline.ts 的 git add/commit/push 都以這個路徑為 `-C`
// 基準，pathspec 一律寫成相對這個路徑的相對路徑。
export const MCPS_ROOT = '/Users/user/aladdin/aladdin_mcps';
...
export const REAL_DIR: Record<'admin' | 'platform', string> = {
    admin: `${ MCPS_ROOT }/aladdin-admin`,
    platform: `${ MCPS_ROOT }/aladdin-platform`,
};
