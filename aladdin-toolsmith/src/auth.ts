 ...
 *
 * 與 aladdin-admin/aladdin-platform 的 H3 名冊機制刻意不同：D9/plan.md 的
 ...
 * launchd/監控探測。
 *
 * 缺設定時 fail-closed：一律 401，不阻擋伺服器啟動（比照 aladdin-admin H1
...
export function createBearerAuthGuard(expectedToken: string | undefined): MiddlewareHandler {
    const expected = expectedToken !== undefined && expectedToken.length > 0
        ...
        : null;
    if (expected === null) {
        console.error('[aladdin-toolsmith auth] 環境變數 TOOLSMITH_API_TOKEN 未設定或為空，所有 /mcp 請求將一律回 401（fail-closed）');
    }
