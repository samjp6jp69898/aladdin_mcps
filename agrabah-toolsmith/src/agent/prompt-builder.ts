 ...
 * agent」明列的要求逐條落地：
 *   - 絕對不要修改 obsidian/mcps/agrabah-admin 與 agrabah-platform 正式目錄。
...
export function buildPrompt(input: BuildPromptInput): string {
    ...
    const realDir = `/Users/user/aladdin/obsidian/mcps/agrabah-${ target }`;
    const otherRealDir = `/Users/user/aladdin/obsidian/mcps/agrabah-${ otherTarget }`;
    const scratchRequestDir = scratchDir;

    return `你是一個在工程師本機執行的 agent，任務是替一位不具備公司原始碼存取權的企劃，
擴充 agrabah-${ target } 這個 MCP server 的能力。你對整個 /Users/user/aladdin monorepo
...
3. **套用 README 第二節「套用這個骨架」列出的樣板**：import session.ts /
   ...
   \`agrabah_${ target }_<動詞>_<名詞>\`，inputSchema 每個欄位都要有 \`.describe()\`。
