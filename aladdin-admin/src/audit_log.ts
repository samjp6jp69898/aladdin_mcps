 ...
 * （H3 名冊**唯一 id**，2026-08-22 使用者裁定改用這個而非 display_name 當歸屬鍵——
 * 見下方 identity/displayName 分離說明）/ displayName（H3 名冊 display_name，僅供人
 * 看的顯示用途，允許重複）/ sourceIp（X-Forwarded-For，ngrok/proxy 帶的來源 IP）/
 ...
 * durationMs。
 *
 * identity 改用唯一 id（2026-08-22，H28 risk_notes (7) 收斂）：auth.ts 檔頭已警告
 * display_name 不保證唯一，若拿它當稽核 log 的歸屬鍵，兩人同名時「操作歸屬到人」
 * （plan.md D2 的核心動機）會失效——兩人的操作在 log 上會混成同一個 identity 值，
 * 事後無法區分。改成寫唯一 id 當 identity、display_name 只另外存一欄供人類閱讀
 * （identity 本身可能是不好認的 slug，如 "h6-review-sec"），兩個欄位互不取代。
 *
