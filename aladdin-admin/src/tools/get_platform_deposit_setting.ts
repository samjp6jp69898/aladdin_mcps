 ...
 *
 * **獨立 review 建議補上的閘門**：雖然這是名義上的 Get method，但它確實會對 DB 產生真實寫入
 * （即使是冪等、固定預設值的寫入），比照本 server 其他寫入類 tool 的既有慣例，補上 H36
 * `assertProdConfirmed` 閘門，不因為它是「Get」就豁免 prod 執行前確認。
 *
    ...
    server.registerTool(
        'aladdin_admin_deposit_admin_get_platform_deposit_setting_for_edit',
        {
            ...
            description:
                ...
                '（callbackBaseUrl），不含任何金鑰/密碼。' +
                '雖然名義上是 Get，本工具仍比照寫入類 tool 的慣例接上 H36 prod confirm 閘門：當這個 server 是' +
                '正式環境（prod）時，執行本工具前必須先用 AskUserQuestion（或功能相同的方式）明確詢問使用者是否' +
                '要在正式環境執行，取得明確同意後才可以帶上 confirm 參數；絕不能自行假設使用者同意。非 prod 環境' +
                '（dev/pre/evi）不需要、也會忽略 confirm 欄位。',
            inputSchema: {
