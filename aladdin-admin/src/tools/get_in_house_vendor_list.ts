/**
 * tools/get_in_house_vendor_list.ts — aladdin_admin_in_house_game_back_office_get_vendor_list
 *
 ...
 * `gameId` 是**範圍鍵**（一個 game 底下可以有多個廠商，如 `get_game_list.ts` 檔頭提到「加拿大 28」
 * dev 實測就有 3 個廠商欄位），不是能鎖定單一目標的欄位；`vendorName` 是模糊比對，同樣無法鎖定單一目標
 * ——**屬 B 級（高風險，無可鎖定單一目標欄位）**。但 B 級規則的重點是「禁止把這類 method 包成內部業務鍵
 * 查找工具」，本 tool 只是單純的分頁清單查詢（不是拿來重新發明業務鍵定位查找），因此不需要套用「強制
 * 逐頁掃描到底」的規則，只需誠實揭露分頁與篩選欄位的限制。
 ...
 * `aladdin_admin_in_house_game_back_office_get_vendor_list` tool：
 ...
 *   - `gameId=1`（對應 get_game_list 實測的「加拿大 28」）：精確篩選只回傳該 gameId 底下的 3 筆，
 *     與無篩選結果的子集關係一致。
 ...
 * 與 aladdin-platform 同名 tool 打同一份底層資料（同一個 in_house_game_vendors 表），兩邊 dev 實測
 * 結果逐筆一致（9 筆總數、id/gameId/name/currencyCode/status/remark 完全相同）。純讀取、無副作用，
 * 符合分類判定。
 */
...
import { InHouseGameVendorListSearch } from '/Users/user/aladdin/abu/admin/src/generated/types.gen.js';
import { remote, withAutoRelogin } from '../session.ts';
import { asTextResult, asErrorResult } from '../mcp_result.ts';

export function registerGetInHouseVendorListTool(server: McpServer): void {
    server.registerTool(
        'aladdin_admin_in_house_game_back_office_get_vendor_list',
        {
            ...
            description:
                ...
                'game 可能對應多個 vendor，非唯一鎖定；可用 aladdin_admin_in_house_game_back_office_get_game_list ' +
                '查得）或 vendorName（模糊比對）篩選，兩者皆可省略（回傳全部）。回傳的 status 是 StatusEnum ' +
                '數字（非文字），id 可用於 UpdateVendorStatus（啟用/停用切換）或 GetVendorEdit 的目標定位。' +
                '無需任何權限節點，任何已登入本後台的使用者皆可呼叫，純讀取、無副作用。',
            inputSchema: {
                gameId: z.number().int().optional().describe('所屬遊戲 id，可用 aladdin_admin_in_house_game_back_office_get_game_list 查得，一個遊戲可能有多個廠商，此欄位不保證鎖定單一結果'),
