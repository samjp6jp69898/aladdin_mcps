/**
 * tools/create_game.ts — aladdin_admin_create_game
 *
 ...
 * 只能對母表已存在的遊戲做「上架到本平台」（見 aladdin-platform 的
    ...
    server.registerTool(
        'aladdin_admin_create_game',
        {
            ...
            description:
                ...
                'gameVendorId 必須是既有場館的 id（可用 aladdin_admin_list_game_vendors 查母表，或用 ' +
                'aladdin_admin_create_game_vendor 的讀回結果拿到）——注意：場館的內部 id 全域共用（admin 建立的 id，platform 端看到的也是同一個數字），' +
                '但新建立的場館預設不會出現在任何 platform 的清單裡，要先由 admin 端呼叫 ' +
                'aladdin_admin_update_platform_game_vendor_status 為該場館啟用特定 platform，' +
                '否則 aladdin-platform 的 aladdin_platform_list_game_vendors 查不到剛建立的場館。' +
                ...
                '建立後要設定圖片，改用 aladdin_admin_edit_game（同一支 gameVendorId+gameId 就能編輯剛建立的遊戲）。' +
