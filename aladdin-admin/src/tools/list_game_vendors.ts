/**
 * tools/list_game_vendors.ts — aladdin_admin_game_vendor_admin_list_game_vendors
 *
 * rajah: GameVendorAdmin.ListGameVendors / ListAllGameVendors
 * （game_back_office.rajah:308, 312）——命名以語意較泛的 ListGameVendors 為準，
...
export function registerListGameVendorsTool(server: McpServer): void {
    server.registerTool(
        'aladdin_admin_game_vendor_admin_list_game_vendors',
        {
            ...
            description:
                ...
                '回傳的 id 就是其他 admin tool 參數名為 gameVendorId 的值（aladdin_admin_game_vendor_admin_list_games、' +
                'aladdin_admin_game_vendor_admin_update_platform_game_vendor_status 都吃這個 id）。' +
                ...
                'aladdin_admin_game_vendor_admin_list_platform_game_vendors 的範圍，那支查的是平台視角、只涵蓋該平台已建立關聯的場館；' +
                '本工具查的是母表本身，包含尚未對任何平台啟用的場館。' +
                'aladdin-platform MCP server 的 aladdin_platform_game_vendor_platform_list_game_vendors 是另一回事：那支列的是「已上架到' +
