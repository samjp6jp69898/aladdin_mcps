/**
 * tools/update_platform_game_vendor_status.ts — aladdin_admin_game_vendor_admin_update_platform_game_vendor_status
 *
...
export function registerUpdatePlatformGameVendorStatusTool(server: McpServer): void {
    server.registerTool(
        'aladdin_admin_game_vendor_admin_update_platform_game_vendor_status',
        {
            ...
            description:
                ...
                'aladdin_admin_game_vendor_admin_create_or_update_game_vendor 新建立的場館預設不會出現在任何 platform 的清單裡，要靠這支把 status ' +
                '設成 enabled 才會出現在該 platform（見 aladdin_admin_game_vendor_admin_list_platform_game_vendors / ' +
                'aladdin-platform 的 aladdin_platform_game_vendor_platform_list_game_vendors）。' +
                'platformId 從 aladdin_admin_platform_management_list_platform_details 取得，gameVendorId 從 aladdin_admin_game_vendor_admin_list_game_vendors ' +
                '（母表全部場館，含尚未對任何平台啟用的）或 aladdin_admin_game_vendor_admin_create_or_update_game_vendor 的讀回結果取得。' +
                'status 合法值（rajah StatusEnum）：unknown/enabled/disabled/frozen/deleted，一般啟用/停用只會用到 ' +
                'enabled/disabled，其餘值語意不在本工具範圍內描述。' +
                '這支 RPC 沒有單筆查詢方法，寫入成功後本工具會用 aladdin_admin_game_vendor_admin_list_platform_game_vendors 的第一頁' +
                ...
                '成任何特定原因。因此 platformId 一律只能用 aladdin_admin_platform_management_list_platform_details 回傳的真實 id，' +
            ...
            inputSchema: {
                ...
                gameVendorId: z.number().int().describe('廠商場館 id，來自 aladdin_admin_game_vendor_admin_create_or_update_game_vendor 或 aladdin_admin_game_vendor_admin_list_platform_game_vendors 的回傳結果'),
