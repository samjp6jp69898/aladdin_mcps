 ...
 * 印一行明確的「已進入拒絕所有請求狀態」。伺服器仍不會因此拒絕啟動（比照 H1
 * 「認證層缺設定不阻擋啟動」）。
 *
 ...
 * 登入態容器（`sessions`，H5）完全不受影響，沒有人需要重新登入；換到的是「撤銷
 * 一定生效」這個不分情境都成立的保證。原本這段驗證要防的事情依然成立，見
 ...
 * 變成 undefined（H5 登入態容器會用它當 key，undefined 不是一個安全的 key），
 * 重則後面比對 token 時對 undefined 呼叫 `Buffer.from()` 直接拋例外——而且是在
 * 本函式 try/catch 之外（比對邏輯屬於每個 request 各自執行），會讓**所有**身分
 * （不只壞掉那筆）全部 500。改成 fail-closed 後這層保護不但還在，而且更強：
 * 不合格的條目連「這次」都不會進入授權路徑，結果是乾淨的 401 而不是 500。
 */
