...
const testTmpDir = mkdtempSync(join(tmpdir(), 'aladdin-admin-files-test-'));
process.env.ALADDIN_ADMIN_FILES_TMP_DIR = testTmpDir;
...
describe('saveUploadedFile', () => {
    ...
    test('超過單檔大小上限：回明確錯誤且不落地', () => {
        process.env.ALADDIN_ADMIN_FILES_MAX_FILE_BYTES = '100';
        try {
            ...
            delete process.env.ALADDIN_ADMIN_FILES_MAX_FILE_BYTES;
        }
    });
    ...
    test('單一身分配額：連續上傳到超過單人上限後第 N 次被拒，cleanup 後恢復正常', () => {
        ...
        process.env.ALADDIN_ADMIN_FILES_MAX_FILE_BYTES = '100000';
        process.env.ALADDIN_ADMIN_FILES_MAX_PER_IDENTITY_BYTES = '10000';
        process.env.ALADDIN_ADMIN_FILES_MAX_TOTAL_BYTES = '100000';
        try {
            ...
            delete process.env.ALADDIN_ADMIN_FILES_MAX_FILE_BYTES;
            delete process.env.ALADDIN_ADMIN_FILES_MAX_PER_IDENTITY_BYTES;
            delete process.env.ALADDIN_ADMIN_FILES_MAX_TOTAL_BYTES;
        }
    });

    test('全域總量配額：不同身分共同計入總量，超過後回明確錯誤且不落地', () => {
        process.env.ALADDIN_ADMIN_FILES_MAX_FILE_BYTES = '100000';
        process.env.ALADDIN_ADMIN_FILES_MAX_PER_IDENTITY_BYTES = '100000';
        process.env.ALADDIN_ADMIN_FILES_MAX_TOTAL_BYTES = '10000';
        try {
    ...
    test('配額計費有區塊大小下限：大量最小合法檔案不能繞過總量配額（磁碟放大攻擊）', () => {
        process.env.ALADDIN_ADMIN_FILES_MAX_FILE_BYTES = '100000';
        process.env.ALADDIN_ADMIN_FILES_MAX_PER_IDENTITY_BYTES = '10000000'; // 夠大，這個測試只測全域總量
        process.env.ALADDIN_ADMIN_FILES_MAX_TOTAL_BYTES = '40960'; // 10 個區塊
        try {
...
describe('cleanupOrphanedDiskFiles — 補「不在 registry 裡的磁碟殘檔」清理路徑（重啟後遺留，H28 磁碟韌性）', () => {
    ...
    test('檔名符合格式但實際是指向目錄外的 symlink：realpath 邊界檢查擋下，不刪除目錄外的目標', () => {
        const outsideDir = mkdtempSync(join(tmpdir(), 'aladdin-admin-files-outside-'));
