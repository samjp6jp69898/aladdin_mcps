...
app.post('/login', async c => {
    ...
    try {
        ...
        if (!result.success) {
            ...
            console.error(`[aladdin-admin http] /login 失敗：identity=${ identity } agrabahIdentifier=${ identifier } errorCode=${ result.errorCode }`);
            return c.json(
                {
                    success: false,
        ...
        console.error(`[aladdin-admin http] /login 成功：identity=${ identity } agrabahIdentifier=${ identifier }`);
        ...
        console.error(`[aladdin-admin http] /login 呼叫 agrabah 時發生未預期例外：identity=${ identity } ${ err instanceof Error ? err.message : String(err) }`);
        ...
        const wrapped = async (...args: unknown[]) => {
            try {
                ...
                if (err instanceof ReloginRequiredError) {
                    ...
                    console.error(`[aladdin-admin http] tool "${ name }" 因登入態失效中止：${ err.message }`);
                ...
                console.error(`[aladdin-admin http] tool "${ name }" ${ label }：${ err instanceof Error ? (err.stack ?? err.message) : String(err) }`);
                throw err;
            }
        };
...
app.onError((err, c) => {
    console.error(`[aladdin-admin http] unhandled error: ${ err instanceof Error ? (err.stack ?? err.message) : String(err) }`);
    return c.text('Internal Server Error', 500);
});

// H36 review 收尾：讓「這個實例的 prod confirm 閘門是否生效」成為開機即可肉眼確認的事實，
// 不必實際打一次寫入 tool 才能知道——這一行印在 stdout（不是稽核 log），部署/除錯時直接看
// launchd 的 out log 或終端機輸出即可核對，不必翻程式碼或猜測 ALADDIN_ADMIN_IS_PROD 有沒有生效。
console.error(`[aladdin-admin MCP] prod 寫入閘門：${ IS_PROD ? '啟用（ALADDIN_ADMIN_IS_PROD=true）' : '停用（非 prod 實例）' }`);
console.error(`[aladdin-admin MCP] http server ready on 127.0.0.1:${ PORT }`);
