...
/**
 * 2026-08-22（使用者裁定，H28 risk_notes (9) 收斂）：hosted 登入態閒置逾時。
 * token 洩漏後的有效窗口原本等於 agrabah JWT 的自然壽命；改成「距上次真的發出過
 * RPC 請求超過 IDLE_TIMEOUT_MS 就視為登出」，把窗口收斂到一段閒置時間。
 *
 * 純「存取當下檢查」而非另開計時器：每次 headerHandler 被呼叫（＝這個身分真的要
 * 發一次 RPC）就順便檢查＋延展 lastActivityMs，不需要 setInterval 主動掃描——沒
 * 有人在用的 session 本來就不會被存取，讓它留在 Map 裡到下次存取才發現已過期並
 * 清掉，結構上等同於一個被動淘汰的快取，不是靠等待判斷正確性（CLAUDE.md 禁止的
 * 是拿 sleep/輪詢間隔規避競態，這裡是用「存取當下與上次存取的真實時間差」做狀態
 * 判斷，兩者不同）。stdio 模式同樣受管轄（STDIO_IDENTITY 也是 Identity 的一種
 * key），行為一致：閒置太久一樣要求重新登入。
 */
const IDLE_TIMEOUT_MS = Number(process.env.ALADDIN_ADMIN_SESSION_IDLE_TIMEOUT_MS ?? 5 * 60 * 1000);

/**
 * per-identity 登入態容器（D2）。只存 agrabah JWT（D3：絕不存帳密）+ 最後一次真的
 * 發出 RPC 的時間戳（lastActivityMs，供閒置逾時判斷）。key 是 H3 名冊唯一 id 或
...
/**
 * 供測試直接灌入登入態，不必真的打一次 RPC 登入（比照 audit_log.ts 的
 * auditLogConfigForTests 慣例）。lastActivityMsAgo 是「距離現在已經過了多久」，
 * 用來直接構造「剛登入」或「已閒置超過門檻」兩種既定狀態，不必真的等待
 * IDLE_TIMEOUT_MS 那麼久（CLAUDE.md 禁止用等待驗證正確性）。僅供測試使用，
 * 正式程式碼路徑一律走 login()。
 */
