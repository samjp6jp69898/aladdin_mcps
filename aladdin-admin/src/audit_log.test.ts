...
const testDir = mkdtempSync(join(tmpdir(), 'aladdin-admin-audit-log-test-'));
const logPath = join(testDir, 'audit.jsonl');
process.env.ALADDIN_ADMIN_AUDIT_LOG_PATH = logPath;
process.env.ALADDIN_ADMIN_AUDIT_LOG_MAX_BYTES = '500'; // 小門檻，方便測輪替不必真的灌大量資料
