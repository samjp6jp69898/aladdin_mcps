#!/bin/zsh
# launchd wrapper：啟動 aladdin-admin hosted MCP server（bun run src/http.ts）。
#
# H13 寫好、手動驗證能起能停；H15 已 launchctl bootstrap 常駐上線，並經
# telegram-dispatcher proxy 對外開放。這支腳本現在是常駐服務的實際進入點——
# 改完程式碼要 `launchctl kickstart -k gui/$(id -u)/com.aladdin.mcp-admin-server`
...
# 預設值，未來任一邊改動容易悄悄漂移。
export ALADDIN_ADMIN_HTTP_PORT=8789

# 刻意不匯出 ALADDIN_ADMIN_USER / ALADDIN_ADMIN_PASSWORD：hosted 模式的登入
