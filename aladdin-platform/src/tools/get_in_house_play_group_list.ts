/**
 * tools/get_in_house_play_group_list.ts — aladdin_platform_in_house_game_back_office_get_play_group_list
 *
 ...
 * 因此本 tool 在 aladdin-admin 與 aladdin-platform 各建一份，這份是 platform 版本。
 *
 ...
 * `aladdin_platform_in_house_game_back_office_get_play_group_list` tool：
 ...
 * 與 aladdin-admin 同名 tool 打同一份底層資料，兩端 4 種情境的 id 列表逐一比對完全一致（同一批
...
import { InHouseGamePlayGroupListSearch } from '/Users/user/aladdin/abu/platform/src/generated/types.gen.js';
...
export function registerGetInHousePlayGroupListTool(server: McpServer): void {
    server.registerTool(
        'aladdin_platform_in_house_game_back_office_get_play_group_list',
        {
            ...
            description:
                ...
                '可用 vendorId（精確比對，但非唯一鎖定）、name（模糊比對）、status（enabled/disabled）篩選，' +
                '皆可省略（回傳全部）。回傳的 status 是 StatusEnum 數字（可能出現 search 篩選不到的' +
                'frozen/deleted 等值），非 search 用的 ActiveStatusEnum。id 可用於 ' +
                'GetTwoEightOddsSetting/GetTwoEightBetLimitSetting/UpdatePlayGroupStatus 等其他 method 的' +
                '目標定位。無需任何權限節點，任何已登入本後台的使用者皆可呼叫，純讀取、無副作用。',
            inputSchema: {
                vendorId: z.number().int().optional().describe('所屬廠商 id，可用 aladdin_platform_in_house_game_back_office_get_vendor_list 查得，一個廠商可能有多個玩法組，此欄位不保證鎖定單一結果'),
