 ...
 * （agrabah/src/servers/sensitive_word_back_office/services/sensitive_word_platform.ts:105-113，
...
export function registerBatchRemoveSensitiveWordTool(server: McpServer): void {
    server.registerTool(
        ...
        async (input) => {
            ...
            const targets = before.rows.filter((r) => existingIds.has(r.id) && input.sensitiveWordIds.includes(r.id));
