 ...
 *
 * agrabah 對應實作：rebate_platform.ts:417-460 methodDeleteRebateConfig，確認有真實 override，
 * 不是 notImplemented stub。
 *
 * 分類：method-category-checklist.md 第 7 節「刪除」。逐條處理：
 * - **軟刪除還是硬刪除（第 7 節第 1 條，明文要求查證、不能憑簽名假設）**：**軟刪除**。
 *   實作是 `rebateConfig.deleted = 1` 後 `updateObject`（rebate_platform.ts:445-447），
 ...
 *   回 `ErrorCode.idNotExists`（11）（:427-429），不是 no-op。
 ...
 *   （:420-429），本 tool 另外在刪除前先呼叫 GetRebateConfigById 讀一次，把即將被刪除的配置
 ...
 *   把 rebate_id 指向這個配置（查 vip 庫 `vip_level_settings`，條件含 `status = enabled`，:429-436）。
 ...
 * 那些紀錄之後在返水紀錄查詢裡的 configName 會回退成字面字串 `{id: 數字}`
 * （rebate_platform.ts:1133，因為名稱 map 是從 rebate_configs 撈的、而該筆仍在表內所以其實查得到，
 * 這裡只是提醒 configName 的來源）。
 *
 * 其他：成功後寫一筆平台稽核 log（PlatformActionIdEnum.rebateConfigDelete，:452-460），
 * 但**沒有**發清快取的 Message（對照 CreateOrUpdateRebateConfig 的更新分支有發
 * OnRebateConfigChangedMessage，:334-338）——這是後端既有的不對稱，如實記錄。
 *
 ...
 * ⚠️ **測試殘留（已知、無法清除）**：後端只有軟刪除、沒有硬刪除 API，所以測試用的 id=1064
 *    會以 deleted=1 的資料列永久留在 dev 的 rebate_configs 表，並持續出現在
 *    get_rebate_config_name_list 的回傳裡。它不會出現在返水配置列表、不綁任何會員、
 *    不影響任何返水計算。名稱已刻意取為「MCP測試勿用_20260828」以便辨識。
 */
    ...
    server.registerTool(
        'aladdin_platform_rebate_platform_delete_rebate_config',
        {
            ...
            description:
                ...
                '預期 get_rebate_config_by_id 讀不到（代表軟刪除生效），若仍讀得到會如實回報異常。',
            inputSchema: {
        ...
        async ({ id }) => {
            ...
            return asTextResult({
                ...
                deletedId: id,
                softDeleteVerified: !stillReadable,
                configSnapshotBeforeDelete: snapshot,
                note: stillReadable
                    ? '⚠️ 刪除的 RPC 回成功，但回讀時仍讀得到這筆配置——與預期（軟刪除後 GetRebateConfigById 因 deleted = 0 條件而讀不到）不符，請人工覆核。'
                    : '刪除成功並已回讀驗證（get_rebate_config_by_id 已讀不到）。這是軟刪除：資料列仍在 rebate_configs 表，'
                        + 'get_rebate_config_name_list 仍會列出這個 id。上方 configSnapshotBeforeDelete 是刪除前的完整內容，'
                        + '本 MCP 沒有復原 tool，需要還原請保留這份快照並走後台 UI。',
            });
        },
    );
}
