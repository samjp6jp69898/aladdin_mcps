 ...
 *
 * agrabah 對應實作：rebate_platform.ts:1021-1186 methodGetRebateRecordList，確認有真實 override
 ...
 * （RebateRecordOptions，rajah:559-590），且其中 `orderId`（返水單號）與 `userId` 都能唯一鎖定
 * 目標，不是只有範圍鍵+分頁的 B 級。依 A 級要求，zod schema 對照 rajah model **全部 13 個欄位
 * 逐一列出**（本 model 沒有 @Hide 欄位，所以沒有「@Hide 但 API 支援」的隱藏篩選鍵要補）。
 *
 * agrabah 實作細節（讀源碼查證，這些都不是從簽名看得出來的）：
 * - **只回「一般返水」**：SQL 固定加 `stepped_settlement_id = 0`（rebate_platform.ts:1035）。
 ...
 *   `ErrorCode.idNotExists`（11）+ message "account not exists"（rebate_platform.ts:1038-1049），
 *   不是回空清單。所以「帳號打錯」與「這個帳號沒有返水紀錄」是兩種不同回應。
 * - **account 與 userId 同時帶會 AND 兩個條件**（:1046 與 :1063 各自 push `user_id = ?`），
 *   兩者指向不同人時必定回 0 筆。本 tool 的 description 已提醒二擇一。
 * - **statuses 的 verified / expired 是查詢期改寫**（:1066-1077）：
 *   `verified` → `status = 1 AND claim_limit_at > NOW()`；
 *   `expired` → `status = 1 AND claim_limit_at <= NOW()`（DB 裡沒有 status=5 這種資料）；
 *   其餘狀態才是直接比對。多選時彼此 OR。
 * - **回傳的 status 也會被就地改寫**（:1139-1141）：status=verified 且 claimLimitAtTimestamp
 ...
 * （rajah:477-512）逐欄檢查後確認沒有這三類欄位，也沒有 token/密碼。金額欄位是純數字。
            ...
            if (r.failed) {
                return asErrorResult(r, {
                    hint: 'errorCode=11（idNotExists）+ message "account not exists" 代表帶的 account 在本平台查無此會員，'
                        + '不代表這個人沒有返水紀錄；其餘錯誤碼請照原樣回報，不要自行改條件重試。',
                });
            }
