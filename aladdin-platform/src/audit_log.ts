/**
 * audit_log.ts — H32 在 aladdin-admin/src/audit_log.ts 拍板的稽核 log 契約，
 * 本檔原樣複製（兩個 server 各自獨立 package、沒有共用套件層，複製一份小檔案
 * 優於臨時造共用套件層——CLAUDE.md Rule 2 Simplicity First，同 auth.ts/session.ts
 * 既有的複製慣例）。把「誰、對哪個路徑/tool 做了什麼、結果如何」寫成 JSON
 ...
 * （ALADDIN_PLATFORM_AUDIT_LOG_MAX_BYTES，預設 10MB）就 closeSync 現有 fd、
...
const LOG_PATH = process.env.ALADDIN_PLATFORM_AUDIT_LOG_PATH
    ?? new URL('../logs/audit.jsonl', import.meta.url).pathname;
const MAX_BYTES = Number(process.env.ALADDIN_PLATFORM_AUDIT_LOG_MAX_BYTES ?? 10 * 1024 * 1024); // 10MB
