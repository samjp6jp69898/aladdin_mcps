...
export const TMP_DIR = process.env.ALADDIN_PLATFORM_FILES_TMP_DIR
    ?? new URL('../tmp-uploads/', import.meta.url).pathname;
mkdirSync(TMP_DIR, { recursive: true });

// D5 前提：遊戲圖示 <1MB；訂一個有餘裕但仍能擋住異常大檔的上限。
function maxFileSizeBytes(): number {
    return Number(process.env.ALADDIN_PLATFORM_FILES_MAX_FILE_BYTES ?? 3 * 1024 * 1024); // 3MB
}

// AC 建議值：單一身分 50MB、全域總量 500MB。
function maxBytesPerIdentity(): number {
    return Number(process.env.ALADDIN_PLATFORM_FILES_MAX_PER_IDENTITY_BYTES ?? 50 * 1024 * 1024);
}

function maxTotalBytes(): number {
    return Number(process.env.ALADDIN_PLATFORM_FILES_MAX_TOTAL_BYTES ?? 500 * 1024 * 1024);
}
    ...
    for (const [ fileId, entry ] of registry) {
        if (nowMs - entry.uploadedAtMs > ttlMs) {
            ...
            try {
                ...
                console.error(`[aladdin-platform files] 清理暫存檔失敗 ${ path }：${ err instanceof Error ? err.message : String(err) }`);
            }
...
export function cleanupOrphanedDiskFiles(ttlMs: number = RETENTION_MS, nowMs: number = Date.now()): number {
    ...
    for (const name of entries) {
        ...
        try {
            ...
            console.error(`[aladdin-platform files] 清理孤兒暫存檔失敗 ${ path }：${ err instanceof Error ? err.message : String(err) }`);
        }
    }
    return removed;
}
