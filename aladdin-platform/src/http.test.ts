...
const auditLines = (await Bun.file(process.env.ALADDIN_PLATFORM_AUDIT_LOG_PATH).text())
...
const child = spawnSync('bun', [ '-e', childScript ], {
    ...
    env: {
        ...
        // session.ts 對 ALADDIN_PLATFORM_API_URL 的啟動檢查。
        ALADDIN_PLATFORM_API_URL: 'http://127.0.0.1:1/never-called-in-this-test',
        ALADDIN_PLATFORM_AUDIT_LOG_PATH: auditLogPath,
        ...
        // 斷言會隨測試檔執行順序時好時壞。
        ALADDIN_PLATFORM_AUDIT_LOG_MAX_BYTES: String(10 * 1024 * 1024),
        ALADDIN_PLATFORM_FILES_TMP_DIR: join(testDir, 'tmp-uploads'),
    },
    timeout: 60_000,
});
