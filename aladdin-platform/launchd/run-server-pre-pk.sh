#!/bin/zsh
# launchd wrapper：啟動 aladdin-platform hosted MCP server 的 pre(cqa)×PK 實例
# （bun run src/http.ts，port 8794，獨立 tokens.pre-pk.json 名冊）。
#
...
# com.aladdin.mcp-platform-pre-pk-server.plist，launchd 實際讀的是
...
# 監聽 port：pre-pk=8794（避開 toolsmith 8788 / admin-dev 8789 /
# platform-dev-pk 8790 / admin-pre 8791 / admin-evi 8792 / platform-dev-6t
# 8793），明講掉不依賴 http.ts 的預設值，理由同既有 run-server*.sh。
export ALADDIN_PLATFORM_HTTP_PORT=8794
...
export ALADDIN_PLATFORM_TOKENS_PATH="$SERVER_DIR/tokens.pre-pk.json"

# 獨立稽核 log 檔，不與其他實例的 logs/audit*.jsonl 共用——不同環境/產品的
# 操作混在同一個檔案會讓稽核追查搞不清楚是哪一份實例的行為。
export ALADDIN_PLATFORM_AUDIT_LOG_PATH="$SERVER_DIR/logs/audit.pre-pk.jsonl"
...
if [ -z "${ALADDIN_PLATFORM_API_URL:-}" ]; then
  echo "ERROR: 環境變數 ALADDIN_PLATFORM_API_URL 未設定或為空。請檢查 plist 的 EnvironmentVariables 是否有 ALADDIN_PLATFORM_API_URL：正本在 $SERVER_DIR/launchd/com.aladdin.mcp-platform-pre-pk-server.plist，但 launchd 讀的是 ~/Library/LaunchAgents/com.aladdin.mcp-platform-pre-pk-server.plist——改完正本要重新 cp 過去，然後 bootout + bootstrap（只用 kickstart 不會重讀 plist）。" >&2
  exit 1
fi
...
for LOG_FILE in "$LOG_DIR/launchd-pre-pk-server.out.log" "$LOG_DIR/launchd-pre-pk-server.err.log"; do
  if [ -f "$LOG_FILE" ]; then
