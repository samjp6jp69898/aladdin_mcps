...
# .mcp.json 的 aladdin-platform server env（stdio 模式沿用至今），所以這裡
...
ALADDIN_PLATFORM_API_URL=$("$JQ" -r '.mcpServers["aladdin-platform"].env.ALADDIN_PLATFORM_API_URL // empty' "$MCP_JSON")
export ALADDIN_PLATFORM_API_URL
...
if [ -z "$ALADDIN_PLATFORM_API_URL" ]; then
  echo "ERROR: 無法從 $MCP_JSON 讀取 mcpServers.aladdin-platform.env.ALADDIN_PLATFORM_API_URL" >&2
  exit 1
fi
