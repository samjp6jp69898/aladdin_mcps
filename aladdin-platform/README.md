...
| `aladdin_platform_agent_platform_view_phone_number` | `AgentPlatform.ViewPhoneNumber` | 開通目前登入操作者查看代理會員手機號碼明碼的授權，效期 1 小時，冪等（重複呼叫只延長效期）；本身不回傳手機號碼，只影響其他清單 method（尚未包裝）的遮罩行為；掛 `@Permission` + `@Totp` |
