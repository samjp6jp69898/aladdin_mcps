...
| `aladdin_platform_live_platform_create_or_update_live_category` | `LivePlatform.CreateOrUpdateLiveCategory` | 新增或修改單一直播分類；先讀 `GetLiveCategories` 現值只覆蓋有帶到的欄位。**⚠️ 這支踩得到 `method-category-checklist.md` 第 4 節模式 2 的「assignKey 數字欄位地雷」**：後端 `DbLiveCategory.create()` 的 `assignKey`（`agrabah/src/database_types/base.ts:6-16`）條件含 `|| source[key] === 0`，protobuf 沒帶到的數字欄位讀到 prototype 預設值 0 就被當成有效值寫進 DB——**只帶 name 呼叫會順手把該分類的 status 改成 unknown(0)**（2026-08-28 dev 用原始 RPC 對 id=1007 負面驗證：1→0，已復原）。本工具一律把現值 status 原樣帶回、且**不開放 status 當輸入**，要啟停請用 `..._update_live_category_status`；字串欄位（四張圖）沒有這個問題但同樣帶回現值。`name` 逐語系合併不刪未帶語系；四個圖片欄位各接受 `path`／`filePath`（stdio 本機檔，工具代為走 `GetUploadImageToken`＋上傳）／`fileId`（hosted）三選一；新增不回傳 id，靠寫入前後 id 集合差異反推，且新建分類狀態必為 enabled（後端寫死）；**⚠️ 後端稽核既有 bug**：新增分類的 actionId 用了 `liveTabCreate` 而非存在的 `liveCategoryCreate`，稽核會記成「新增直播頁籤」；本 service 無 delete method，建立後只能停用。2026-08-28 dev 實測：不存在 id 擋下、缺 name 擋下、新增 id=1007 反推新 id 且 status=1、只改 zh-TW 驗證逐語系合併且 status/圖片皆保留、圖片三選一互斥檢查、真實上傳一張 PNG 取得 `/static/live/...` 路徑並寫入成功；測試資料 id=1007 已停用（無法刪除，屬已知殘留） |
