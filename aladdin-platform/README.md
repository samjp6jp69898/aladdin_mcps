...
| `aladdin_platform_in_house_game_back_office_list_available_game_codes` | `InHouseGameBackOffice.ListAvailableGameCodes` | 列出自研（in-house）二八槓遊戲可用的 gameCode 全集，無參數、不分頁，直接回傳後端固定的 GameCodeEnum 靜態列舉（不查 DB）；2026-08-25 dev 實測回傳 CND28/ORG28/BIT28/MIN28 共 4 筆，兩次呼叫結果一致 |
