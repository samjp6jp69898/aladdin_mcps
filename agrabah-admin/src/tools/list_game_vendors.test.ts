...
/**
 * agrabah_admin_list_game_vendors 的註冊契約與分流測試。
 *
 * 這支 tool 的存在理由是「admin 端沒有任何一支能列出母表全部場館的查詢」——真人測試中
 * agent 因此跑去 agrabah-platform server 找廠商 id。所以要釘住的正是那兩件事：
 ...
 */
process.env.AGRABAH_ADMIN_API_URL = 'http://127.0.0.1:1/never-called-in-this-test';
...
describe('註冊契約', () => {
    test('tool 名稱是 agrabah_admin_list_game_vendors', () => {
        expect(tool.name).toBe('agrabah_admin_list_game_vendors');
    });
    ...
    test('description 講明母表視角、id 語意、與相鄰 tool 的分工（含不要跨到 platform server 找場館）', () => {
        ...
        expect(d).toContain('agrabah_admin_list_platform_game_vendors');
        expect(d).toContain('agrabah_platform_list_game_vendors');
    });
});
