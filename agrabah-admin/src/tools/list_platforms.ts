/**
 * tools/list_platforms.ts — agrabah_admin_list_platforms
 *
...
export function registerListPlatformsTool(server: McpServer): void {
    server.registerTool(
        'agrabah_admin_list_platforms',
        {
            title: 'List all platforms',
            description:
                '列出全部平台（rajah: PlatformManagement.ListPlatformDetails，admin.rajah:116；刻意未綁 @Permission，' +
                '因平台清單是跨一級菜單共用的下拉來源）。用途：取得平台的 id / code，供 agrabah_admin_list_platform_game_vendors ' +
                '與 agrabah_admin_update_platform_game_vendor_status 這兩支需要 platformId 參數的 tool 使用——這兩支不接受 ' +
