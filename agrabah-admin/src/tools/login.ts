/**
 * tools/login.ts — agrabah_admin_login
 */
...
export function registerLoginTool(server: McpServer): void {
    server.registerTool(
        'agrabah_admin_login',
        {
            ...
            inputSchema: {
                identifier: z.string().optional().describe('登入帳號，留空則用環境變數 AGRABAH_ADMIN_USER'),
                password: z.string().optional().describe('登入密碼，留空則用環境變數 AGRABAH_ADMIN_PASSWORD'),
