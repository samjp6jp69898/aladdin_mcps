/**
 * tools/list_game_vendors.ts — agrabah_admin_list_game_vendors
 *
...
export function registerListGameVendorsTool(server: McpServer): void {
    server.registerTool(
        'agrabah_admin_list_game_vendors',
        {
            ...
            description:
                ...
                '回傳的 id 就是其他 admin tool 參數名為 gameVendorId 的值（agrabah_admin_list_vendor_games、' +
                'agrabah_admin_update_platform_game_vendor_status 都吃這個 id）。' +
                ...
                'agrabah_admin_list_platform_game_vendors 的範圍，那支查的是平台視角、只涵蓋該平台已建立關聯的場館；' +
                '本工具查的是母表本身，包含尚未對任何平台啟用的場館。' +
                'agrabah-platform MCP server 的 agrabah_platform_list_game_vendors 是另一回事：那支列的是「已上架到' +
