...
function maxFileSizeBytes(): number {
    return Number(process.env.AGRABAH_ADMIN_FILES_MAX_FILE_BYTES ?? 3 * 1024 * 1024); // 3MB
}

// AC 建議值：單一身分 50MB、全域總量 500MB。
function maxBytesPerIdentity(): number {
    return Number(process.env.AGRABAH_ADMIN_FILES_MAX_PER_IDENTITY_BYTES ?? 50 * 1024 * 1024);
}

function maxTotalBytes(): number {
    return Number(process.env.AGRABAH_ADMIN_FILES_MAX_TOTAL_BYTES ?? 500 * 1024 * 1024);
}
    ...
    for (const [ fileId, entry ] of registry) {
        if (nowMs - entry.uploadedAtMs > ttlMs) {
            ...
            try {
                ...
                console.error(`[agrabah-admin files] 清理暫存檔失敗 ${ path }：${ err instanceof Error ? err.message : String(err) }`);
            }
