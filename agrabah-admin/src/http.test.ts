 ...
 */
const testDir = mkdtempSync(join(tmpdir(), 'agrabah-admin-http-test-'));
const auditLogPath = join(testDir, 'audit.jsonl');
const TOOL_NAME = 'agrabah_admin_list_vendor_games';
...
const auditLines = (await Bun.file(process.env.AGRABAH_ADMIN_AUDIT_LOG_PATH).text())
...
const child = spawnSync('bun', [ '-e', childScript ], {
    ...
    env: {
        ...
        // session.ts 對 AGRABAH_ADMIN_API_URL 的啟動檢查。
        AGRABAH_ADMIN_API_URL: 'http://127.0.0.1:1/never-called-in-this-test',
        AGRABAH_ADMIN_AUDIT_LOG_PATH: auditLogPath,
        ...
        // 斷言會隨測試檔執行順序時好時壞。
        AGRABAH_ADMIN_AUDIT_LOG_MAX_BYTES: String(10 * 1024 * 1024),
        AGRABAH_ADMIN_FILES_TMP_DIR: join(testDir, 'tmp-uploads'),
    },
    timeout: 60_000,
});
