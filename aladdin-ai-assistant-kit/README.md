...
裝好之後，**日常使用不需要打任何指令**，這份 kit 裡有給你雙擊的啟動器：
...
它會自動切換到正確的資料夾、檢查設定是否完整、然後開啟 Claude。
如果有東西還沒設定好（例如帳密還沒填），它會直接告訴你該做什麼，而不是丟一堆錯誤訊息。

> **Mac 首次使用要多做一步**：Mac 為了安全，預設不允許雙擊執行剛下載的腳本。
> 請在終端機執行一次下面這行（只需要做這一次）：
> ```bash
> chmod +x ~/aladdin-kit/開始使用-Mac.command
> ```
> 之後就能一直雙擊使用了。如果雙擊時 Mac 說「無法打開，因為來自未識別的開發者」，
> 請在該檔案上按右鍵 → 開啟 → 再點一次「開啟」。

下面的安裝步驟只需要做一次；做完之後，以後每天就是雙擊啟動器而已。
...
5. 讓啟動器可以雙擊執行（只需做這一次）：
   ...
   ```
6. 之後每次要用，**直接雙擊 `開始使用-Mac.command`** 就好。
   （也可以照舊在終端機 `cd ~/aladdin-kit` 之後執行 `claude`，兩種都行。）
...
4. 複製 `.env.example` 成 `.env`（同一個資料夾內直接複製貼上、改檔名即
   ...
   **注意**：如果你的文字編輯器把檔案存成 Windows 慣用的 CRLF 換行，
   ...
   協助確認 `.env` 的存檔格式。
5. 之後每次要用，**直接雙擊 `開始使用-Windows.bat`** 就好——它會自動切到
   正確的資料夾並開啟 Claude。
   （也可以照舊自己開 Claude Code 並切到 `C:\aladdin-kit`，兩種都行。）
